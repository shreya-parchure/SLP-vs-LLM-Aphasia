...
1) ASR transcription of speech from people with aphasia (PWA) using Whisper large language model. The python notebook `Guide: Using Whisper LLM to Transcribe Speech in Aphasia.ipynb` includes tutorial-style comments and simplified code that can run in any browser using Google colaboratory, which includes the optimized settings for anyone to use for speech to text.
